...
\eq{
\mc{M}_g = \faktor{\set{metrics h_{ab} }}{\set{Diff}\times\set{Weyl}}
}
A useful result is 
\eq{
s = |\mc{M}_g| = \left\{ \begin{array}{lc} 0 & g=0 \\ 2 & g=1 \\ 6g-6 & g\geq 2 \end{array} \right.
}
...
U = \begin{pmatrix} a & b \\ c & d \end{pmatrix} \quad a
}
We can undo that change by also changing 
\eq{
(n,m) \to (n,m) U^{-1}
}
so for $(n,m)$ to be integers we require $a,b,c,d \in \mbb{Z}$, i.e $U\in SL(2,\mbb{Z})$. Our moduli soace is the 
\eq{
\mc{M}_1 = \faktor{UHP}{SL(2,\mbb{Z})}
}
