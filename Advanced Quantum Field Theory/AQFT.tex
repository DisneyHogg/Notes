...
&= \frac{(-1)^m}{\hbar^m} \Pffaf(A)
}
where 
\eq{
\Pffaf(A) = \highlight{\frac{1}{2^m m!}} \eps^{a_1 \dots a_{2m}} A_{a_1 a_2}A_{a_3 a_3} \dots A_{a_{2m-1} a_{2m}} 
}
\begin{ex}
Show $\Pffaf(A) = \pm \sqrt{\det A}$
\end{ex}

\begin{example}
\eq{
\Pffaf \begin{pmatrix} 0 & -a \\ a & 0 \end{pmatrix} = a
}
\end{example}
Then
\eq{
