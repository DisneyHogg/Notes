...
If, during expansion $a \to e^N a$, is it said that \bam{N e-fold of expansion have occurred}. $N$ can we written as a variable 
\eq{
...
\eq{
\Rightarrow \Phi_k = -2\mc{R}_k(0) \frac{\sin(\frac{k\tau}{\sqrt{3}}) -\frac{k\tau}{\sqrt{3}} \cos(\frac{k\tau}{\sqrt{3}})}{(\frac{k\tau}{\sqrt{3}})^3}
}
Recalling that during radiation domination $\tau = \frac{1}{\mc{H}}$, on subhorizon scales $k\tau = \frac{k}{\mc{H}} \gg 1$ so 
\eq{
\Phi_k \approx 6 \mc{R}_k(0) \frac{\cos(\frac{k\tau}{\sqrt{3}})}{(k\tau)^2}
}
...
Writing this as 
\eq{
\left[ \delta_r(k,\tau) + 4\Phi_MD(k) \right]^{\prime \prime} + \frac{1}{3} k^2 \left[ \delta_r(k,\tau) + 4\Phi_{MD}(k) \right] = 0
}
...
\Rightarrow \frac{ d\log (aE)}{d\tau } - \frac{d\Phi}{d\tau} + 2\Phi^\prime
}
...
\eq{
S = \int d\tau \, d^3 x \, a^4 \left( \frac{1}{2} \frac{1}{a^2} [(\del_\tau \phi)^2  + |\grad \phi|^2 ] - V \right)
}
...
&= \frac{k^3}{2\pi^2} |f_k(\tau)|^2 \\
&= \frac{k^3}{2\pi^2} \frac{1}{4k} \left( 1 + \frac{1}{k^2 \tau^2} \right) \\
...
\eq{
\mc{R}(\bm{x}) = - \mc{H} \frac{\delta \delta(\bm{x})}{\bar{\phi}^\prime} \\
